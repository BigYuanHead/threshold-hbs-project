...
class WinternitzUntrustedServer:
    def __init__(self, public_params, backend=None):
    ...
    def sign(self, message: bytes, keyid: int, parties: list) -> dict:
        ...
        if len(parties) != self.public_params["n_parties"]:
            raise ValueError("this winternitz protocol requires all n parties")
        ...
        combined_signature = self.backend.combine_sig_shares(signature_shares)
